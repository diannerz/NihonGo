...
// email must end with .com (per requirement)
if (!filter_var($email, FILTER_VALIDATE_EMAIL) || !preg_match('/\.com$/i', $email)) {
    http_response_code(400);
    echo json_encode(['error' => 'Email must be valid and end with .com']);
    exit;
}
...
if (strlen($password) < 6 || !preg_match('/[A-Za-z]/', $password) || !preg_match('/\d/', $password)) {
    http_response_code(400);
    echo json_encode(['error' => 'Password must be at least 6 chars and include letters and numbers.']);
    exit;
}

try {
    // check duplicates
    $stmt = $pdo->prepare('SELECT id, username, email FROM users WHERE username = :u OR email = :e');
